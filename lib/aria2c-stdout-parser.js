...
module.exports = {
    ...
    aria2cTorrentStatusStdoutParser(stdout) {
        ...
        if (isInitializing || isChecksumming) {
            ...
            const [bytesProcessed, sizeBytesInitialization] = [Number(bytesProgressMatch[4]), Number(bytesProgressMatch[5])];
            return {
                statusType: torrentStatusType.initializing,
                sizeBytes,
                sizeBytesInitialization,
                bytesProcessed,
                completionPercentage: utils.number.truncateToDecimals(bytesProcessed / sizeBytesInitialization, useDecimalPlaces)
            };
        }
